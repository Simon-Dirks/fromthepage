...
class Work < ApplicationRecord
  ...
  belongs_to :next_untranscribed_page,
             foreign_key: 'next_untranscribed_page_id',
             class_name: 'Page',
             optional: true
  ...
  has_many :metadata_description_versions,
  ...
  scope :incomplete_transcription,
        -> do
          where(supports_translation: false)
            .joins(:work_statistic)
            .where.not(work_statistics: { complete: 100 })
        end
  scope :incomplete_translation,
        -> do
          where(supports_translation: true)
            .joins(:work_statistic)
            .where.not(work_statistics: { translation_complete: 100 })
        end
  scope :incomplete_description,
        -> { where(description_status: DescriptionStatus::NEEDS_WORK) }
  ...
  def merge_metadata(include_user = false)
    metadata = []
    metadata += JSON[self.original_metadata] if self.original_metadata
    work_metadata =
      self.attributes.select do |k, v|
        PUBLIC_ATTRIBUTES.include?(k) && !v.blank?
      end
  ...
  def emended_transcription_plaintext
    self
      .pages
      .select { |page| !page.status_blank? }
      .map { |page| page.emended_transcription_plaintext }
      .join("\n\n\n")
  end
  ...
  def update_deed_collection
    deeds
      .where.not(collection_id: collection_id)
  ...
  def thumbnail
    if !self.picture.blank?
      ...
      unless self.pages.count == 0
        set_featured_page if self.featured_page.nil?
  ...
  def update_next_untranscribed_pages
    ...
    unless document_sets.empty?
      document_sets.each { |ds| ds.set_next_untranscribed_page }
    end
  end
